// data/shaders/vulkan/rt/FsrCommon.glsl
//
...
//
//   ffx_denoiser_reflections_config.h  -> the FSR_* tuning constants
...
#ifndef COIN_FSR_COMMON_GLSL
#define COIN_FSR_COMMON_GLSL

// --- ffx_denoiser_reflections_config.h --------------------------------------
#define FSR_GAUSSIAN_K 3.0
#define FSR_RADIANCE_WEIGHT_BIAS 0.6
#define FSR_RADIANCE_WEIGHT_VARIANCE_K 0.1
#define FSR_AVG_RADIANCE_LUMINANCE_WEIGHT 0.3
#define FSR_PREFILTER_VARIANCE_WEIGHT 4.4
#define FSR_REPROJECT_SURFACE_DISCARD_VARIANCE_WEIGHT 1.5
#define FSR_PREFILTER_VARIANCE_BIAS 0.1
#define FSR_PREFILTER_NORMAL_SIGMA 512.0
#define FSR_PREFILTER_DEPTH_SIGMA 4.0
#define FSR_DISOCCLUSION_NORMAL_WEIGHT 1.4
#define FSR_DISOCCLUSION_DEPTH_WEIGHT 1.0
#define FSR_DISOCCLUSION_THRESHOLD 0.9
#define FSR_REPROJECTION_NORMAL_SIMILARITY_THRESHOLD 0.9999
#define FSR_SAMPLES_FOR_ROUGHNESS(r) (1.0 - exp(-(r) * 100.0))
...
uint fsrPackFloat16(vec2 v)
{
    return packHalf2x16(v);
}

vec2 fsrUnpackFloat16(uint a)
{
    return unpackHalf2x16(a);
}

uvec2 fsrPackFloat16x4(vec4 v)
{
    return uvec2(fsrPackFloat16(v.xy), fsrPackFloat16(v.zw));
}

vec4 fsrUnpackFloat16x4(uvec2 a)
{
    return vec4(fsrUnpackFloat16(a.x), fsrUnpackFloat16(a.y));
}
...
float fsrComputeTemporalVariance(vec3 historyRadiance, vec3 radiance)
{
    float historyLuminance = fsrLuminance(historyRadiance);
    float luminance = fsrLuminance(radiance);
...
vec3 fsrClipAABB(vec3 aabbMin, vec3 aabbMax, vec3 prevSample)
{
...
#define FSR_LOCAL_NEIGHBORHOOD_RADIUS 2
float fsrLocalNeighborhoodKernelWeight(float i)
{
    const float radius = FSR_LOCAL_NEIGHBORHOOD_RADIUS + 1.0;
    return exp(-FSR_GAUSSIAN_K * (i * i) / (radius * radius));
}

#endif // COIN_FSR_COMMON_GLSL
