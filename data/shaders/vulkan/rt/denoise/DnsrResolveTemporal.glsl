// data/shaders/vulkan/rt/DnsrResolveTemporal.glsl
//
...
// and _resolve_temporal.h; see DnsrCommon.glsl for the shared helpers/license).
//
...
//
// Inputs come from DnsrPrefilter.glsl (prefiltered radiance/variance), the path
...
layout(push_constant) uniform DnsrTemporalPush {
...
ivec2 dnsrUvToTexel(vec2 uv, uvec2 screen)
{
...
void main()
{
    ...
    if (inBounds) {
        ivec2 ht = dnsrUvToTexel(historyUv, screen);
        ...
        float disocclusion = 1.0
            * exp(-abs(1.0 - max(0.0, dot(normalize(curNormal),
                                           normalize(hNormal))))
                  * DNSR_DISOCCLUSION_NORMAL_WEIGHT)
            * exp(-abs(hDepth - depth) / max(depth, 1.0e-3)
                  * DNSR_DISOCCLUSION_DEPTH_WEIGHT);

        if (disocclusion >= DNSR_DISOCCLUSION_THRESHOLD) {
    ...
    vec3 clippedHistory = dnsrClipAABB(mean - colorStd, mean + colorStd,
                                      historyRadiance);
    ...
    float outVariance = mix(dnsrComputeTemporalVariance(curRadiance,
                                                       clippedHistory),
                            curVariance, weight);
