// src/rendering/SoRTXRenderBackend/SoRTXRenderBackendPick.cpp
...
SoRTXRenderBackend::createPickResources()
{
  ...
  if (vkCreateShaderModule(this->device, &sci, this->allocator,
                           &this->pickModule) != VK_SUCCESS) {
    SoDebugError::postWarning("SoRTXRenderBackend::createPickResources",
                              "pick shader module creation failed");
  ...
  if (vkCreateComputePipelines(this->device, VK_NULL_HANDLE, 1, &cpci,
                               this->allocator,
                               &this->pickPipeline) != VK_SUCCESS) {
    this->destroyPickResources();
    return false;
  }
                            ...
                            float tMax, RTPickHit & out)
{
  ...
  vkResetCommandBuffer(this->pickCommandBuffer, 0);

  VkCommandBufferBeginInfo bi {};
  bi.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
  bi.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
  if (vkBeginCommandBuffer(this->pickCommandBuffer, &bi) != VK_SUCCESS) {
    return false;
  }
  ...
  if (vkQueueSubmit(this->queue, 1, &si, this->pickFence) != VK_SUCCESS) {
    return false;
  }
  if (vkWaitForFences(this->device, 1, &this->pickFence, VK_TRUE,
