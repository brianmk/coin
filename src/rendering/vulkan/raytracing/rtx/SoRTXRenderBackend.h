// src/rendering/vulkan/raytracing/rtx/SoRTXRenderBackend.h
...
struct RTXCachedGeometry {
  ...
  // Offset/count of this command's triangle texture coordinates in the UV
  // pool (UINT32_MAX = not uploaded yet; count is the triangle count).
  uint32_t uvPoolOffset = 0xFFFFFFFFu;
  uint32_t uvCount = 0;
...
/*!
  ...
  triangleData + pbr + textureData + textureLayers = 848 + 64 = 912 bytes.
  Defined here so the backend can own a reusable std::vector<RTMaterial>
  scratch buffer without keeping a per-frame heap allocation.
*/
struct RTMaterial {
  float diffuse[4];
  float ambient[4];
  ...
  // Optional material textures.  textureData: x = triangle-UV pool offset
  // (the UV pool mirrors the normal pool), y = UV count (0 = command has no
  // texture coordinates), z = normal-map strength, w = emissive-map intensity.
  // textureLayers holds the sampler2DArray layer (set 0, binding 17) for the
  // base colour, roughness, normal and emissive maps; a channel with no map is
  // -1.  pbr.w carries the roughness-map strength (pbr.xyz keep their meaning).
  float textureData[4];
  float textureLayers[4];
  // Optical (transmission/glass) model, mirroring SoRenderIR::SoMaterialBlock:
  // x = transmission IOR, y = Beer-Lambert absorption strength, z = transmission
  // (opacity), w = reserved.  The dielectric BSDF reads x/y; z is the same
  // transmittance diffuse.a carries.
  float optical[4];
};
...
private:
  ...
  // Wait for a queue to drain (vkQueueWaitIdle), reporting a named failure
  // through emitError.  Defaults to this backend's queue; a null queue is a
  // successful no-op (mirroring the old `if (queue != VK_NULL_HANDLE)` guards).
  bool drainQueue(const char * context, VkQueue queue = VK_NULL_HANDLE);
  // Free descriptor set(s) back to their pool, reporting a named failure.
  bool freeDescriptorSets(VkDescriptorPool pool, uint32_t count,
                          const VkDescriptorSet * sets, const char * context);
  bool freeDescriptorSet(VkDescriptorPool pool, VkDescriptorSet set,
                         const char * context);
  ...
  bool recordAccelerationStructures(const SoDrawList & drawlist,
                                    ...
                                    VkCommandBuffer cmd);
  bool updatePathTracingState(const SoDrawList & drawlist,
                              const SoRenderParams & params,
                              const SoVulkanRenderTarget & target,
                              VkCommandBuffer cmd);
  ...
  // Submit a one-shot command buffer for the denoiser output copy on the
  // graphics queue, then block until it completes so the result is published
  // before the present that samples it.  (The copy used to be submitted to a
  // dedicated compute queue under FC_VULKAN_ASYNC_COMPUTE, but the host blocks
  // on completion anyway, so there was no overlap -- only extra cross-queue
  // ownership and synchronization hazards.  It now always runs on graphics.)
  ...
  // Retained for the device queue probe; the denoiser copy itself now runs on
  // the graphics queue (see submitDenoiseCopy()).
  ...
  // PBR gate override: -1 = defer to the material's own physicalMaterial flag
  // (the raster backend's rule); 0/1 = force off/on via FC_VULKAN_RT_PBR for
  // A/B comparison.
  int rtPbrForce = -1;
  ...
  // Per-triangle texture coordinates (set 0, binding 18), three vec4 per
  // triangle (u, v, 0, 0), mirroring the normal pool.  Built alongside the
  // normals so a shaded hit can barycentric-interpolate its UV.  Per-command
  // offsets ride in RTMaterial::textureData (x/y).
  VkBuffer uvPoolBuffer = VK_NULL_HANDLE;
  VmaAllocation uvPoolMemory = VK_NULL_HANDLE;
  void * uvPoolMapped = nullptr;
  VkDeviceSize uvPoolCapacity = 0;
  VkDeviceSize uvPoolUsed = 0;
  bool ensureUvPoolCapacity(VkDeviceSize bytes);
  VkDeviceSize appendTriangleUvs(const SoRenderCommand & command,
                                 RTXCachedGeometry & entry);

  // Per-vertex object-space tangents (set 0, binding 19), three vec4 per
  // triangle (xyz = face tangent, w = bitangent sign), written in lockstep with
  // the UV pool (the shader indexes it with the SAME RTMaterial::textureData.x
  // offset).  Only consumed for the normal map (textureLayers.z).
  VkBuffer tangentPoolBuffer = VK_NULL_HANDLE;
  VmaAllocation tangentPoolMemory = VK_NULL_HANDLE;
  void * tangentPoolMapped = nullptr;
  VkDeviceSize tangentPoolCapacity = 0;
  VkDeviceSize tangentPoolUsed = 0;
  bool ensureTangentPoolCapacity(VkDeviceSize bytes);

  // Material texture array (set 0, binding 17): one sampler2DArray whose layers
  // hold every distinct material texture in the scene.  Per-command layer
  // indices ride in RTMaterial::textureLayers; -1 means the channel has no map
  // and the shader samples nothing.  Rebuilt only when the scene's texture set
  // changes (textureSetHash), never per frame.
  VkImage textureArrayImage = VK_NULL_HANDLE;
  VmaAllocation textureArrayMemory = VK_NULL_HANDLE;
  VkImageView textureArrayView = VK_NULL_HANDLE;
  VkSampler textureArraySampler = VK_NULL_HANDLE;
  uint32_t textureArrayLayers = 0;
  uint32_t textureArrayExtent = 0;
  uint64_t textureSetHash = 0;
  uint32_t textureArrayCommandCount = 0;
  // Four layer indices (base, roughness, normal, emissive) per command.
  std::vector<int32_t> commandTextureLayers;
  bool ensureTextureArray(uint32_t layers, uint32_t extent);
  bool uploadMaterialTextures(const SoDrawList & drawlist, VkCommandBuffer cmd);
  ...
  // --- Denoiser backends (OIDN / RTX-OptiX / DNSR) -------------------------
  ...
  // present binding 5.
  enum DenoiseKind { DenoiseNone = 0, DenoiseOidn, DenoiseRtx, DenoiseDnsr };
  ...
  //! worker skips its own CPU normalize/downsample and only runs OIDN/DNSR.
  ...
  // The "dnsr" denoiser slot runs the DNSR denoiser on the GPU over the path
  ...
  // a spatial prefilter (DnsrPrefilter.glsl) and a temporal reproject+resolve
  // (DnsrResolveTemporal.glsl) that consumes the motion buffer and keeps
  // radiance/variance/sample-count history.  createDnsrPipeline() builds both
  // descriptor sets + pipelines and the intermediate/history buffers;
  // dispatchDnsrDenoise() records both passes and writes denoisedBuffer.  The
  // bodies are compiled out when COIN_BUILD_DNSR_DENOISER=0, so the declarations
  // stay unguarded (mirroring the OIDN/RTX pattern).
  //
  // Prefilter (spatial) stage.
  VkDescriptorSetLayout dnsrSetLayout = VK_NULL_HANDLE;
  VkPipelineLayout dnsrPipelineLayout = VK_NULL_HANDLE;
  VkPipeline dnsrPipeline = VK_NULL_HANDLE;
  VkDescriptorSet dnsrDescriptorSet = VK_NULL_HANDLE;
  VkShaderModule dnsrModule = VK_NULL_HANDLE;
  // Temporal reproject + resolve stage.
  VkDescriptorSetLayout dnsrTemporalSetLayout = VK_NULL_HANDLE;
  VkPipelineLayout dnsrTemporalPipelineLayout = VK_NULL_HANDLE;
  VkPipeline dnsrTemporalPipeline = VK_NULL_HANDLE;
  VkDescriptorSet dnsrTemporalDescriptorSet = VK_NULL_HANDLE;
  VkShaderModule dnsrTemporalModule = VK_NULL_HANDLE;
  ...
  VkBuffer dnsrPrefilterRad = VK_NULL_HANDLE;
  VmaAllocation dnsrPrefilterRadMem = VK_NULL_HANDLE;
  VkBuffer dnsrPrefilterVar = VK_NULL_HANDLE;
  VmaAllocation dnsrPrefilterVarMem = VK_NULL_HANDLE;
  VkBuffer dnsrHistRad[2] = {VK_NULL_HANDLE, VK_NULL_HANDLE};
  VmaAllocation dnsrHistRadMem[2] = {VK_NULL_HANDLE, VK_NULL_HANDLE};
  VkBuffer dnsrHistVar[2] = {VK_NULL_HANDLE, VK_NULL_HANDLE};
  VmaAllocation dnsrHistVarMem[2] = {VK_NULL_HANDLE, VK_NULL_HANDLE};
  // The history sample count is packed into dnsrHistRad[].w (radiance only uses
  // .rgb), so no separate per-pixel count buffer is allocated.
  VkBuffer dnsrHistPos = VK_NULL_HANDLE;
  VmaAllocation dnsrHistPosMem = VK_NULL_HANDLE;
  VkBuffer dnsrHistNrm = VK_NULL_HANDLE;
  VmaAllocation dnsrHistNrmMem = VK_NULL_HANDLE;
  //! Index of the current (previous-frame) history slot; the resolve reads
  //! [dnsrHistIndex] and writes [1 - dnsrHistIndex], then flips.
  int dnsrHistIndex = 0;
  //! World-space camera origin of the frame being denoised (for the temporal
  //! disocclusion depth test).
  float dnsrCameraPos[3] = {0.0f, 0.0f, 0.0f};
  bool dnsrPipelineReady = false;
  ...
  //! Build the DNSR descriptor set + compute pipeline from the embedded
  //! DnsrPrefilter SPIR-V.  Defined unconditionally in SoRTXRenderBackendDnsr.cpp
  //! (body compiled out when COIN_BUILD_DNSR_DENOISER=0).
  bool createDnsrPipeline();
  //! Release the DNSR pipeline/layout/descriptor set/shader module.
  void destroyDnsrResources();
  //! Run the DNSR denoise pass over the device-local G-buffers at \a w x \a h
  ...
  bool dispatchDnsrDenoise(uint32_t w, uint32_t h);
};
