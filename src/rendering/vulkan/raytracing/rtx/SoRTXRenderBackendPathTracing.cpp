// src/rendering/vulkan/raytracing/rtx/SoRTXRenderBackendPathTracing.cpp
  ...
  const SoVulkanRenderTarget & target, VkCommandBuffer cmd)
{
  ...
  if (!this->updatePathTracingState(drawlist, params, target, cmd)) {
    // Buffer (re)creation failed and already emitted an error; abort the frame
    // rather than recording against half-updated accumulation state.
    return false;
  }
  ...
  if (needMaterials || needTlas) {
    if (needMaterials) {
      ...
      // Refresh the material texture array before the material records are
      // packed: updateMaterials() reads the per-command layer indices this
      // resolves, and the texture upload must be recorded (outside the render
      // pass) before the trace phase samples it.
      if (!this->uploadMaterialTextures(drawlist, cmd)) {
        this->emitError(
          "recordAccelerationStructures: failed to upload material textures");
        return false;
      }
  ...
  if (this->frameMapped) {
    ...
    // Snapshot the world-space camera origin for the DNSR/DNSR temporal pass,
    ...
    this->dnsrCameraPos[0] = frame.cameraPos[0];
    this->dnsrCameraPos[1] = frame.cameraPos[1];
    this->dnsrCameraPos[2] = frame.cameraPos[2];
