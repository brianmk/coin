// src/rendering/vulkan/raytracing/rtx/SoRTXRenderBackendBuffers.cpp
                                            ...
                                            VmaAllocation & memory)
{
  ...
  const SoVulkan::Result create = SoVulkanShared::createVmaBuffer(
    this->vmaAllocator, desc, "createDeviceLocalBuffer", buffer, memory);
  if (!create.isOk()) {
                                            ...
                                            void ** mapped)
{
  ...
  desc.flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT |
               VMA_ALLOCATION_CREATE_MAPPED_BIT;
  VmaAllocationInfo allocationInfo {};
  const SoVulkan::Result create = SoVulkanShared::createVmaBuffer(
    this->vmaAllocator, desc, "createHostVisibleBuffer", buffer, memory,
    &allocationInfo);
  if (!create.isOk()) {
    this->emitError(create.message().c_str());
    return false;
...
SoRTXRenderBackend::createPathTracingBuffers(uint32_t width, uint32_t height)
{
  ...
  // OIDN/RTX/DNSR device+filter themselves.  When a denoiser is active the
  ...
  // host-side OIDN/DNSR backends support scaling -- the RTX interop path stays
  // native (scale 1) because it reads the G-buffers device-to-device and
  // needs a GPU downsample that does not exist there.  The DNSR/DNSR pass is
  ...
  if (!this->createDenoiseBackend()) {
    ...
    this->drainQueue("createPathTracingBuffers");
    this->destroyDenoiser();

    // The persistent mapping comes from VMA_ALLOCATION_CREATE_MAPPED_BIT, so
    // there is no vmaMapMemory to balance and calling vmaUnmapMemory here
    // would trip VMA's "Unmapping allocation not previously mapped" assert.
    // vmaDestroyBuffer below releases the mapping with the allocation.
    this->activeCounterMapped = nullptr;
    ...
    this->freeDescriptorSets(this->descriptorPool, descriptorSetCount,
                             descriptorSets, "createPathTracingBuffers");
