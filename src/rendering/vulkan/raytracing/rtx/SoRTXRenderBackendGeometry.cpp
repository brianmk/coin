// src/rendering/vulkan/raytracing/rtx/SoRTXRenderBackendGeometry.cpp
...
SoRTXRenderBackend::ensureNeePoolCapacity(VkDeviceSize bytes)
{
  return this->ensurePoolCapacity(bytes, this->neePoolBuffer,
                                  this->neePoolMemory,
                                  this->neePoolMapped,
                                  this->neePoolCapacity,
                                  this->neePoolUsed, false);
}
...
SoRTXRenderBackend::flushPendingDestroys(bool waitForQueue)
{
  ...
  if (waitForQueue && this->queue != VK_NULL_HANDLE &&
    ...
    // The batch about to be released may still be referenced by an
    // in-flight submission; report rather than destroy silently.
    this->drainQueue("flushPendingDestroys");
  }
...
{
  // Compaction is a best-effort memory optimization that is retried every
  // frame while it keeps failing; report the reason only on the first failure
  // so a broken driver cannot flood the log.
  static bool reported = false;
  const auto reportVk = [this](const char * what, VkResult res) {
    if (reported) return;
    reported = true;
    this->emitError((std::string("compactBlas: ") + what + " failed: "
                     + SoVulkanShared::vkResultName(res)).c_str());
  };
  const auto reportMsg = [this](const char * msg) {
    if (reported) return;
    reported = true;
    this->emitError((std::string("compactBlas: ") + msg).c_str());
  };

  if (entry.blas == VK_NULL_HANDLE) return true;
  if (!this->ensureCompactQueryPool()) {
    reportMsg("could not create the compaction fence/query pool");
    return false;
  }

  VkCommandBuffer cmd = this->beginTransientCommandBuffer();
  if (cmd == VK_NULL_HANDLE) {
    reportMsg("could not allocate the compaction command buffer");
    return false;
  }
  ...
  if (endRes != VK_SUCCESS) {
    reportVk("vkEndCommandBuffer (size query)", endRes);
    return false;
  }
  VkSubmitInfo si {};
  ...
  const VkResult sizeSubmit =
    vkQueueSubmit(this->queue, 1, &si, this->compactFence);
  if (sizeSubmit != VK_SUCCESS) {
    reportVk("vkQueueSubmit (size query)", sizeSubmit);
    return false;
  }
  const VkResult sizeWait = vkWaitForFences(this->device, 1, &this->compactFence,
                                            VK_TRUE, UINT64_MAX);
  if (sizeWait != VK_SUCCESS) {
    reportVk("vkWaitForFences (size query)", sizeWait);
    return false;
  }
  const VkResult resetRes = vkResetFences(this->device, 1, &this->compactFence);
  if (resetRes != VK_SUCCESS) {
    reportVk("vkResetFences (size query)", resetRes);
    return false;
  }
  const VkDeviceSize origSize = entry.blasSize;
  VkDeviceSize compactSize = 0;
  const VkResult queryRes = vkGetQueryPoolResults(
    this->device, this->compactQueryPool, 0, 1, sizeof(compactSize),
    &compactSize, sizeof(compactSize), VK_QUERY_RESULT_WAIT_BIT);
  if (queryRes != VK_SUCCESS) {
    reportVk("vkGetQueryPoolResults", queryRes);
    return false;
  }
  ...
  if (!this->createDeviceLocalBuffer(
        ...
        cBuf, cMem)) {
    reportMsg("could not allocate the compacted-AS buffer");
    return false;
  }
  ...
  const VkResult createAsRes = vkCreateAccelerationStructureKHR(
    this->device, &asCI, this->allocator, &cAs);
  if (createAsRes != VK_SUCCESS || cAs == VK_NULL_HANDLE) {
    vmaDestroyBuffer(this->vmaAllocator, cBuf, cMem);
    if (createAsRes != VK_SUCCESS) {
      reportVk("vkCreateAccelerationStructureKHR", createAsRes);
    }
    else {
      reportMsg("vkCreateAccelerationStructureKHR returned a null handle");
    }
    return false;
  }
  ...
  if (cmd2 == VK_NULL_HANDLE) {
    reportMsg("could not allocate the compaction copy command buffer");
  }
  else {
    ...
    const VkResult endRes2 = vkEndCommandBuffer(cmd2);
    if (endRes2 != VK_SUCCESS) {
      reportVk("vkEndCommandBuffer (compact copy)", endRes2);
    }
    else {
      VkSubmitInfo si2 {};
      ...
      const VkResult submitRes2 =
        vkQueueSubmit(this->queue, 1, &si2, this->compactFence);
      if (submitRes2 != VK_SUCCESS) {
        reportVk("vkQueueSubmit (compact copy)", submitRes2);
      }
      else {
        const VkResult waitRes2 = vkWaitForFences(
          this->device, 1, &this->compactFence, VK_TRUE, UINT64_MAX);
        if (waitRes2 != VK_SUCCESS) {
          reportVk("vkWaitForFences (compact copy)", waitRes2);
        }
        else {
          const VkResult resetRes2 =
            vkResetFences(this->device, 1, &this->compactFence);
          if (resetRes2 != VK_SUCCESS) {
            reportVk("vkResetFences (compact copy)", resetRes2);
          }
          else {
            ok = true;
          }
                                     ...
                                     bool refit)
{
  ...
  // A failed *build* (not a refit) must not leave the vertex, index or BLAS
  // buffer it created behind.  createDeviceLocalBuffer() nulls the member
  // handle on entry, so if a half-created set survived, the next attempt would
  // overwrite the handles and leak the allocations -- and because entry.blas
  // stays null until the build completes, the entry never compares equal and
  // is retried every frame.  Release whatever the entry currently owns (a
  // no-op on a clean build), null the handles, and clear refitPending so the
  // next attempt does a full rebuild instead of a MODE_UPDATE against a null
  // BLAS.  Refits reuse the existing buffers, so they must not release them.
  auto releasePartialBuild = [this, &entry, refit]() -> bool {
    ...
    if (blas == VK_NULL_HANDLE && blasBuffer == VK_NULL_HANDLE &&
  ...
  // not build a BLAS whose normals would be missing.
  {
    ...
    if (normalTriangles > 0 &&
      ...
      return releasePartialBuild();
    }
    // Texture coordinates for the same triangles (no-op for an untextured or
    // texcoord-less command; the pool allocation failure is non-fatal -- the
    // hit then shades with the scalar diffuse).
    this->appendTriangleUvs(command, entry);
  }
  ...
  const uint32_t maxPrimitives =
    indexed ? entry.indexCount / 3 : entry.vertexCount / 3;
  if (maxPrimitives == 0) return releasePartialBuild();
  ...
  if (!refit && sizeInfo.accelerationStructureSize == 0) {
    // The driver could not size the build (invalid geometry/format).  Report
    // it instead of failing later on a zero-size buffer.
    this->emitError(
      "blasBuildOrRefit: vkGetAccelerationStructureBuildSizesKHR returned a "
      "zero accelerationStructureSize");
    return releasePartialBuild();
  }
  ...
  if (!refit) {
    ...
    asCI.type = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR;
    const VkResult createAsRes = vkCreateAccelerationStructureKHR(
      this->device, &asCI, this->allocator, &entry.blas);
    if (createAsRes != VK_SUCCESS) {
      this->emitError(("blasBuildOrRefit: vkCreateAccelerationStructureKHR "
                       "failed: "
                       + SoVulkanShared::vkResultName(createAsRes)).c_str());
      return releasePartialBuild();
    }
    ...
    if (entry.devAddr == 0) {
      this->emitError(
        "blasBuildOrRefit: vkGetAccelerationStructureDeviceAddressKHR "
        "returned a null device address");
      return releasePartialBuild();
    }
    buildInfo.dstAccelerationStructure = entry.blas;
  }
                              ...
                              VkCommandBuffer cmd)
{
  ...
  if (this->instanceCount > 0 && sizeInfo.accelerationStructureSize == 0) {
    this->emitError(
      "buildTlas: vkGetAccelerationStructureBuildSizesKHR returned a zero "
  ...
  if (this->tlas == VK_NULL_HANDLE ||
    ...
    const VkResult createTlasRes = vkCreateAccelerationStructureKHR(
      this->device, &asCI, this->allocator, &this->tlas);
    if (createTlasRes != VK_SUCCESS) {
      this->emitError(("buildTlas: vkCreateAccelerationStructureKHR failed: "
                       + SoVulkanShared::vkResultName(createTlasRes)).c_str());
      // The handle was just allocated for this failed attempt and no
      // submission references it yet; release it here, or the next frame
      // (this->tlas is still null) would overwrite this->tlasBuffer via
      // createDeviceLocalBuffer() and leak the allocation.
      this->tlas = VK_NULL_HANDLE;
      vmaDestroyBuffer(this->vmaAllocator, this->tlasBuffer, this->tlasMemory);
      this->tlasBuffer = VK_NULL_HANDLE;
      this->tlasMemory = VK_NULL_HANDLE;
...
SoRTXRenderBackend::updateMaterials(const SoDrawList & drawlist)
{
  ...
  // PBR gate: by default defer to the material's own physicalMaterial flag,
  // matching the raster backend, so one authored material shades the same in
  // both.  FC_VULKAN_RT_PBR, when set, forces it on/off for A/B comparison.
  this->rtPbrForce = -1;
  if (SoVulkanShared::envString("FC_VULKAN_RT_PBR") != nullptr) {
    this->rtPbrForce = COIN_VULKAN_ENV_FLAG("FC_VULKAN_RT_PBR") ? 1 : 0;
  }
  {
  ...
  // Parse a debug float override, rejecting malformed text.  strtof() with a
  // null end pointer returns 0 on failure, which would silently turn a typo in
  // the env var into a 0 override.
  const auto parseEnvFloat = [](const char * text, float & out) -> bool {
  ...
  if (const char * metalEnv = SoVulkanShared::envString("FC_VULKAN_RT_METAL")) {
    if (parseEnvFloat(metalEnv, this->rtMetalValue)) {
      this->rtMetalOverride = true;
    }
    else {
      this->emitError("FC_VULKAN_RT_METAL is not a number; override ignored");
    }
  }
  if (const char * roughEnv = SoVulkanShared::envString("FC_VULKAN_RT_ROUGH")) {
    if (parseEnvFloat(roughEnv, this->rtRoughValue)) {
      this->rtRoughOverride = true;
    }
    else {
      this->emitError("FC_VULKAN_RT_ROUGH is not a number; override ignored");
    }
  }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // Canonical material mapping, shared with the raster backend.
    SoRenderIR::SoMaterialBlock materialBlock;
    SoRenderIR::packMaterialBlock(materialBlock, material);
    // The viewer's global glass settings are the default optics.  Apply them
    // only when the material did not author its own (optical.w flag): otherwise
    // a per-material dielectric would be silently replaced by the global
    // setting.  The precedence rule lives in SoRenderIR::resolveOptical so the
    // unit tests exercise the exact code the backend runs.
    SoRenderIR::resolveOptical(materialBlock,
                               this->ptGlassIor,
                               this->ptGlassAbsorption);
    std::memcpy(out.diffuse, materialBlock.diffuse, sizeof(out.diffuse));
    std::memcpy(out.ambient, materialBlock.ambient, sizeof(out.ambient));
    std::memcpy(out.specular, materialBlock.specular, sizeof(out.specular));
    std::memcpy(out.emissive, materialBlock.emissive, sizeof(out.emissive));
    std::memcpy(out.params, materialBlock.params, sizeof(out.params));
    std::memcpy(out.pbr, materialBlock.pbr, sizeof(out.pbr));
    // Optical model from the shared block (IOR/absorption/transmission).  These
    // are the material's own optics; the backend's global glass setting is the
    // default the IR is populated with, so the shader reads them per material.
    std::memcpy(out.optical, materialBlock.optical, sizeof(out.optical));
    ...
    if (cacheFound != this->commandToCache.end()) {
      ...
      // Texture coordinates (set by appendTriangleUvs()): x/y are the UV pool
      // offset/count the shader interpolates for texture sampling.  z/w carry
      // the normal-map and emissive-map strengths (the roughness strength rides
      // in pbr[3]), so the shader can blend the maps like the raster backend.
      out.textureData[0] = entry.uvCount > 0
        ? static_cast<float>(entry.uvPoolOffset) : 0.0f;
      out.textureData[1] = static_cast<float>(entry.uvCount);
      out.textureData[2] = material.normalStrength;
      out.textureData[3] = material.emissiveIntensity;
    }

    // Material texture-array layers (base colour, roughness, normal,
    // emissive) resolved by uploadMaterialTextures(); -1 = channel absent.
    const bool haveLayers =
      (this->commandTextureLayers.size() ==
       static_cast<size_t>(drawlist.getNumCommands()) * 4);
    for (int channel = 0; channel < 4; ++channel) {
      out.textureLayers[channel] =
        haveLayers
          ? static_cast<float>(
              this->commandTextureLayers[static_cast<size_t>(i) * 4 + channel])
          : -1.0f;
    }
    // Optional PBR (metallic-roughness) parameters.  The shared packer set
    // pbr[2] from the material's physicalMaterial flag (matching the raster
    // backend); FC_VULKAN_RT_PBR forces the gate and FC_VULKAN_RT_METAL /
    // FC_VULKAN_RT_ROUGH override the values.  When off, usePbr stays 0 and
    // every shading path above falls back to the legacy model.
    const bool usePbr = this->rtPbrForce >= 0
      ? (this->rtPbrForce != 0)
      : material.physicalMaterial;
    out.pbr[2] = usePbr ? 1.0f : 0.0f;
    ...
    // Roughness-map strength (the shader blends the sampled map by this).
    out.pbr[3] = material.roughnessStrength;
    ...
    // shader: litColor += ambientLight * materialAmbient).  The convention
    // lives in SoRenderIR::effectiveMaterialAmbient so both backends agree.
    const SbVec3f ambient = SoRenderIR::effectiveMaterialAmbient(
      effective.ambient, material.ambient);
    out.ambient[0] = ambient[0];
    out.ambient[1] = ambient[1];
    out.ambient[2] = ambient[2];
    out.ambient[3] = 1.0f;

    // Light set via the shared block packer (world-space, so toEye == nullptr).
    SoLightingBlock lightBlock;
    const int lightCount =
      SoRenderIR::fillLightingBlock(lightBlock, effective, nullptr);
    out.params[2] = static_cast<float>(lightCount);
    std::memcpy(out.lightType, lightBlock.lightType,
                sizeof(out.lightType));
    std::memcpy(out.lightColor, lightBlock.lightColor,
                sizeof(out.lightColor));
    std::memcpy(out.lightDirection, lightBlock.lightDirection,
                sizeof(out.lightDirection));
    std::memcpy(out.lightPosition, lightBlock.lightPosition,
                sizeof(out.lightPosition));
    std::memcpy(out.lightAttenuation, lightBlock.lightAttenuation,
                sizeof(out.lightAttenuation));
    std::memcpy(out.lightSpot, lightBlock.lightSpotParams,
                sizeof(out.lightSpot));
  }
