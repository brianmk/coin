// src/rendering/vulkan/raster/SoVulkanRenderBackendCommand.cpp
//
                                              ...
                                              const float * projFloats)
{
  ...
  // One material mapping shared with the path tracer.  The block's diffuse is
  // ignored here (raster passes diffuse through the push constant to keep
  // per-vertex/per-face colour working); every other field maps 1:1 onto the
  // DrawBlock material slots.
  SoRenderIR::SoMaterialBlock materialBlock;
  SoRenderIR::packMaterialBlock(materialBlock, material);
  std::memcpy(ubo.emissive, materialBlock.emissive, sizeof(ubo.emissive));
  std::memcpy(ubo.materialAmbient, materialBlock.ambient,
              sizeof(ubo.materialAmbient));
  std::memcpy(ubo.materialSpecular, materialBlock.specular,
              sizeof(ubo.materialSpecular));
  std::memcpy(ubo.materialParams, materialBlock.params,
              sizeof(ubo.materialParams));
  std::memcpy(ubo.materialPbr, materialBlock.pbr, sizeof(ubo.materialPbr));
  std::memcpy(ubo.materialMapParams, materialBlock.mapParams,
              sizeof(ubo.materialMapParams));
  // A draw-level unlit override wins over the material's own shading model.
  ubo.materialParams[3] = unlit ? 0.0f : materialBlock.params[3];
                                           ...
                                           VulkanRecordContext & ctx)
{
  ...
  // Set 2: the optional PBR maps (defaults when the command has none).
  VkDescriptorSet mapSet = this->textureCache.resolveMaps(command);
  if (mapSet == VK_NULL_HANDLE) {
    mapSet = this->textureCache.defaultMapDescriptorSet();
  }
  ...
  // Two dynamic offsets: set 0 (lighting) and set 1 (per-draw UBO); set 2 has
  // no dynamic descriptor and takes none.
  ...
  if (ctx.lastBoundLightingOffset != lightingDynamicOffset ||
      ...
      ctx.lastBoundMapSet != mapSet) {
    // First draw of a new lighting handle / texture set / map bundle: bind all
    // three sets with their dynamic offsets in one call (also covers the
    // frame's first draw).
    const VkDescriptorSet all[3] = {this->lightingDescriptorSet, textureSet,
                                    mapSet};
    vkBackendTrace(this->uboFrameIndex, "draw.bindDescSets3",
                   "slot=%u", slotIndex);
    vkCmdBindDescriptorSets(ctx.buffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS,
                            this->pipelineLayout, 0, 3, all, 2,
                            bindingOffsets);
    ...
    // Same lighting handle + texture set + maps as the previous draw: only the
    // per-draw UBO dynamic offset advances.  Re-bind set 1 alone (sets 0 and 2
    // stay bound from the last bind) instead of re-emitting all sets.
                                         ...
                                         VulkanRecordContext & ctx)
{
  ...
  if (!useWideLine) {
    ...
    if (instByteOffset + sizeof(float) * 16 > this->instanceModelCapacity) {
      // The ring is pre-sized to hold every slot; reaching here means a prior
      // grow (swapLightingBuffer) failed.  Report once, since the draw would
      // otherwise lose its instance transform silently.
      static bool reported = false;
      if (!reported) {
        reported = true;
        this->emitError(
          "recordDrawCommand: instance-model ring too small for this slot; "
          "instance transform omitted");
      }
      return;
    }
    SbMat mm;
...
SoVulkanRenderBackend::beginCommandBuffer()
{
  ...
  VK_CHECK_BOOL(vkBeginCommandBuffer(cmd, &bi), "beginCommandBuffer");
  return true;
}
...
SoVulkanRenderBackend::endAndSubmit()
{
  ...
  if (endRc != VK_SUCCESS) {
    // A failed vkEndCommandBuffer leaves the command buffer in the recording
    // (or invalid) state.  This slot is not pending, so beginFrame() will not
    // reset it on the next reuse; without an explicit reset the next
    // beginCommandBuffer() would call vkBeginCommandBuffer on a recording
    // buffer, which is invalid and wedges the slot.  Do the same end+reset the
    // framebuffer-failure path already performs.
    this->emitError(("endAndSubmit: vkEndCommandBuffer failed: "
                     + SoVulkanShared::vkResultName(endRc)).c_str());
    const VkResult resetRc = vkResetCommandBuffer(cmd, 0);
    if (resetRc != VK_SUCCESS) {
      this->emitError(("endAndSubmit: vkResetCommandBuffer after a failed end "
                       "failed: "
                       + SoVulkanShared::vkResultName(resetRc)).c_str());
    }
  ...
  if (submitRc != VK_SUCCESS) {
    ...
    // would wait forever on the next reuse of this slot.  Mark the slot as not
    // pending so the wait is skipped; the command buffer is in the executable
    // state, which the next vkBeginCommandBuffer() implicitly resets, so it is
    // safe to reuse.  A submission failure (typically device loss) leaves the
    // backend unusable anyway.
    this->emitError(("endAndSubmit: vkQueueSubmit failed: "
                     + SoVulkanShared::vkResultName(submitRc)).c_str());
    this->frameRing.setPending(slot, false);
    return false;
  }
