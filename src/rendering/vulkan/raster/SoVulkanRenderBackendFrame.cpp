// src/rendering/vulkan/raster/SoVulkanRenderBackendFrame.cpp
//
                               ...
                               uint64_t hashA, uint64_t hashB)
{
  ...
  if (ma.metalness != mb.metalness) return false;
  if (ma.roughness != mb.roughness) return false;
  if (ma.physicalMaterial != mb.physicalMaterial) return false;
  ...
  // Optional secondary PBR maps: pointer identity plus sampler state.
  const auto sameMap = [](const SoTextureData & x, const SoTextureData & y) {
    return x.pixels == y.pixels && x.width == y.width &&
      x.height == y.height && x.numComponents == y.numComponents &&
      x.model == y.model && x.minFilter == y.minFilter &&
      x.magFilter == y.magFilter && x.wrapS == y.wrapS &&
      x.wrapT == y.wrapT &&
      memcmp(&x.blendColor[0], &y.blendColor[0], sizeof(SbVec4f)) == 0;
  };
  if (!sameMap(ma.roughnessTexture, mb.roughnessTexture)) return false;
  if (!sameMap(ma.normalTexture, mb.normalTexture)) return false;
  if (!sameMap(ma.emissiveTexture, mb.emissiveTexture)) return false;
  if (ma.roughnessStrength != mb.roughnessStrength) return false;
  if (ma.normalStrength != mb.normalStrength) return false;
  if (ma.emissiveIntensity != mb.emissiveIntensity) return false;
  return true;
}
...
static uint64_t vkBatchKey(const SoRenderCommand & a, uint64_t contentHash)
{
  ...
  uint32_t d0, d1, d2, d3, shin, alphaRef, lw, metal, rough;
  xu(&d0, &dif[0]); xu(&d1, &dif[1]); xu(&d2, &dif[2]); xu(&d3, &dif[3]);
  xu(&shin, &a.material.shininess);
  xu(&metal, &a.material.metalness);
  xu(&rough, &a.material.roughness);
  xu(&alphaRef, &a.state.alphaTest.reference);
  xu(&lw, &r.lineWidth);
  h ^= (uint64_t)(d0 ^ d1 ^ d2 ^ d3 ^ shin ^ metal ^ rough ^ alphaRef ^ lw) << 17;
  h ^= (uint64_t)(a.material.physicalMaterial ? 1u : 0u) << 33;
  // Secondary PBR maps: same map pointers => same material bundle.
  h ^= (uint64_t)((uintptr_t)a.material.roughnessTexture.pixels >> 4) << 20;
  h ^= (uint64_t)((uintptr_t)a.material.normalTexture.pixels >> 4) << 27;
  h ^= (uint64_t)((uintptr_t)a.material.emissiveTexture.pixels >> 4) << 34;
  h ^= (uint64_t)(a.material.flags
                 & (SO_MAT_HAS_ROUGHNESS_MAP | SO_MAT_HAS_NORMAL_MAP
                    | SO_MAT_HAS_EMISSIVE_MAP)) << 41;
  return h;
}
...
SoVulkanRenderBackend::shutdown()
{
  ...
  if (this->mapSetLayout != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(this->device, this->mapSetLayout,
                                 this->allocator);
    this->mapSetLayout = VK_NULL_HANDLE;
  }
                                      ...
                                      const bool overlaysOnly)
{
  ...
  if (!this->renderPasses.ensureFramebuffer(
    ...
    // every later beginCommandBuffer() will fail.  Report a reset failure,
    // otherwise the next frame's begin fails with no trace of the origin.
    const VkCommandBuffer cb = this->currentCommandBuffer();
    const VkResult endRes = vkEndCommandBuffer(cb);
    if (endRes != VK_SUCCESS) {
      this->emitError(("failed to end the frame command buffer after a "
                       "framebuffer failure: "
                       + SoVulkanShared::vkResultName(endRes)).c_str());
    }
    const VkResult resetRes = vkResetCommandBuffer(cb, 0);
    if (resetRes != VK_SUCCESS) {
      this->emitError(("failed to reset the frame command buffer after a "
                       "framebuffer failure: "
                                            ...
                                            VkFramebuffer framebuffer)
{
  ...
  // A reset failure (device lost) would make every later begin on this
  // secondary fail with no trace of the origin; surface it instead.
  VK_CHECK_BOOL(vkResetCommandBuffer(secondary, 0), "recordSecondaryChunk");
  ...
  const VkResult endRes = vkEndCommandBuffer(secondary);
  if (endRes != VK_SUCCESS) {
    // A secondary left in the recording state must never be handed to
    // vkCmdExecuteCommands; report the reason and fail the chunk so the caller
    // falls back to inline recording instead of submitting an invalid buffer.
    this->emitError(
