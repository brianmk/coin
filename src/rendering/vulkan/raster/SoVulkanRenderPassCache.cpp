// src/rendering/vulkan/raster/SoVulkanRenderPassCache.cpp
//
...
void
SoVulkanRenderPassCache::setErrorSink(std::function<void(const char *)> sink)
{
  this->errorSink_ = std::move(sink);
}

void
SoVulkanRenderPassCache::reportError(const char * message) const
{
  if (this->errorSink_) {
    this->errorSink_(message);
  }
}
                                          ...
                                          VkRenderPass & pass)
{
  ...
  const VkResult result =
    vkCreateRenderPass(this->device_, &ci, this->allocator_, &pass);
  if (result != VK_SUCCESS) {
    pass = VK_NULL_HANDLE;
    this->reportError(
      ("SoVulkanRenderPassCache: vkCreateRenderPass failed: "
                                           ...
                                           VkRenderPass renderPass)
{
  ...
  fci.layers = 1;
  const VkResult fbResult = vkCreateFramebuffer(
    this->device_, &fci, this->allocator_, &this->framebuffer_);
  if (fbResult != VK_SUCCESS) {
    this->framebuffer_ = VK_NULL_HANDLE;
    this->reportError(
      ("SoVulkanRenderPassCache: vkCreateFramebuffer failed: "
       + SoVulkanShared::vkResultName(fbResult)).c_str());
    return false;
  }
  this->framebufferPass_ = renderPass;
  this->framebufferColorImage_ = target->colorImage;
