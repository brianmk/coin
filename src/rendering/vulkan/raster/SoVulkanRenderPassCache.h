// src/rendering/vulkan/raster/SoVulkanRenderPassCache.h
//
...
public:
  ...
  */
  ...
  //! Register the diagnostics sink used to report render-pass/framebuffer
  //! creation failures (the owning backend forwards it to its own log).
...
private:
  ...
  //! Diagnostics sink; empty means creation failures are silent.
  std::function<void(const char *)> errorSink_;
  //! Forward a failure message to the sink (no-op when unset).
