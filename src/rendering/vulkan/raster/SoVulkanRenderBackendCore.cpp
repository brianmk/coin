// src/rendering/vulkan/raster/SoVulkanRenderBackendCore.cpp
//
...
#include "rendering/vulkan/generated/shaders/visual/Fragment.spv.h"
#include "rendering/vulkan/generated/shaders/visual/Vertex.spv.h"
#include "rendering/vulkan/generated/shaders/wide-line/WideLineFragment.spv.h"
#include "rendering/vulkan/generated/shaders/wide-line/WideLineInstancedVertex.spv.h"
#include "rendering/vulkan/generated/shaders/wide-line/WideLineVertex.spv.h"
#include "rendering/vulkan/generated/shaders/geometry-lod/SubPixelCull.spv.h"
#include "rendering/vulkan/generated/shaders/visual/BackgroundVertex.spv.h"
#include "rendering/vulkan/generated/shaders/visual/BackgroundFragment.spv.h"
...
bool
SoVulkanRenderBackend::drainQueue(const char * context, VkQueue queue)
{
...
bool
SoVulkanRenderBackend::freeDescriptorSets(VkDescriptorPool pool, uint32_t count,
...
bool
SoVulkanRenderBackend::freeDescriptorSet(VkDescriptorPool pool,
                                         VkDescriptorSet set,
                                         const char * context)
{
...
SoVulkanRenderBackend::setMaxFramesInFlight(const uint32_t count)
{
  ...
  // Pre-allocate the resized lighting rings BEFORE committing the new count.
  // Their geometry is derived from the in-flight count, so an allocation
  // failure must abort with the old count and buffers intact -- otherwise
  // maxFramesInFlight would advance past a too-small ring and the per-frame
  // ring-offset math would run out of bounds.  The queue is idle here (waited
  // above), so nothing can reference a not-yet-installed buffer.
  VkBuffer newLighting = VK_NULL_HANDLE;
  VmaAllocation newLightingMemory = nullptr;
  void * newLightingMapped = nullptr;
  VkBuffer newConst = VK_NULL_HANDLE;
  VmaAllocation newConstMemory = nullptr;
  void * newConstMapped = nullptr;
  const bool resizeLighting =
    this->isInitialized() && this->lightingBuffer != VK_NULL_HANDLE;
  const bool resizeConst =
    this->isInitialized() && this->lightingConstBuffer != VK_NULL_HANDLE;

  if (resizeLighting) {
    const VkDeviceSize totalBytes =
      static_cast<VkDeviceSize>(count) * this->uboSlotsPerFrame *
      this->uboSlotStride;
    if (!this->buffers.createMapped(totalBytes,
                                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                    newLighting, newLightingMemory,
                                    &newLightingMapped)) {
      this->emitError("setMaxFramesInFlight: failed to resize lighting UBO; "
                      "keeping the current in-flight count");
      return;
    }
  }
  if (resizeConst) {
    const VkDeviceSize totalBytes =
      static_cast<VkDeviceSize>(count) * kLightingConstSlotsPerFrame *
      this->lightingConstStride;
    if (!this->buffers.createMapped(totalBytes,
                                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                    newConst, newConstMemory, &newConstMapped)) {
      this->emitError("setMaxFramesInFlight: failed to resize lighting "
                      "constant UBO; keeping the current in-flight count");
      if (newLighting != VK_NULL_HANDLE) {
        vmaDestroyBuffer(this->vmaAllocator, newLighting, newLightingMemory);
      }
      return;
    }
  }

  // All new resources exist; commit the count now.
  ...
  if (this->isInitialized()) {
    ...
    if (!this->allocateFrameResources()) {
      ...
      if (!this->allocateFrameResources()) {
        ...
        // The pre-allocated rings were never installed; free them before
        // shutdown() destroys the VMA allocator.
        if (newLighting != VK_NULL_HANDLE) {
          vmaDestroyBuffer(this->vmaAllocator, newLighting, newLightingMemory);
          newLighting = VK_NULL_HANDLE;
        }
        if (newConst != VK_NULL_HANDLE) {
          vmaDestroyBuffer(this->vmaAllocator, newConst, newConstMemory);
          newConst = VK_NULL_HANDLE;
        }
        this->shutdown();
        return;
      }
      // Rolled back to previousCount: the existing lighting rings are already
      // the right size, so discard the pre-allocated (wrong-count) ones.
      if (newLighting != VK_NULL_HANDLE) {
        vmaDestroyBuffer(this->vmaAllocator, newLighting, newLightingMemory);
        newLighting = VK_NULL_HANDLE;
      }
  ...
  // Committed to `count`: install the resized rings.  swapLightingBuffer()
  // waits the (already idle) in-flight frames and repoints every descriptor
  // set at the new buffer.
  if (newLighting != VK_NULL_HANDLE) {
    this->swapLightingBuffer(newLighting, newLightingMemory, newLightingMapped,
                             this->uboSlotsPerFrame);
  }
  if (newConst != VK_NULL_HANDLE) {
    this->lightingConstMaxSlots =
      this->maxFramesInFlight * kLightingConstSlotsPerFrame;
    this->swapLightingConstBuffer(newConst, newConstMemory, newConstMapped);
  }
}
...
SoVulkanRenderBackend::initialize(const SoRenderBackendInitParams & params)
{
  ...
  this->renderPasses.setErrorSink(
  ...
  texContext.allocateMapDescriptorSet =
    [this](const VkImageView views[3], const VkSampler samplers[3],
           VkDescriptorSet & set, VkDescriptorPool & pool) {
      return this->allocateMapDescriptorSet(views, samplers, set, pool);
    };
  texContext.freeDescriptorSet =
    [this](VkDescriptorSet set, VkDescriptorPool pool) {
      this->freeDescriptorSet(pool, set, "freeDescriptorSet");
...
SoVulkanRenderBackend::waitForInFlightFrames()
{
  ...
  if (!this->frameRing.waitAll(this->device)) {
    // A failed wait (typically device lost) means one or more fences may still
    // be unsignaled: report it rather than proceeding to rewrite/teardown
    // resources an in-flight submission could still reference.
    this->emitError("waitForInFlightFrames: vkWaitForFences failed");
  }
}
...
SoVulkanRenderBackend::createDescriptorSetLayout()
{
  ...
  // Set 2: the optional PBR texture maps -- roughness (0), normal (1) and
  // emissive (2), all combined image samplers read by the fragment stage.
  VkDescriptorSetLayoutBinding mapBindings[3] {};
  for (int i = 0; i < 3; ++i) {
    mapBindings[i].binding = static_cast<uint32_t>(i);
    mapBindings[i].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    mapBindings[i].descriptorCount = 1;
    mapBindings[i].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    mapBindings[i].pImmutableSamplers = nullptr;
  }
  VkDescriptorSetLayoutCreateInfo mapCi {};
  mapCi.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
  mapCi.bindingCount = 3;
  mapCi.pBindings = mapBindings;
  return vkCreateDescriptorSetLayout(this->device, &mapCi, this->allocator,
                                     &this->mapSetLayout) == VK_SUCCESS;
}
...
SoVulkanRenderBackend::createDescriptorPool()
{
  ...
  // One sampled descriptor for every set-1 (base texture) and three for every
  // set-2 (roughness/normal/emissive maps): 2048 + 3 * 2048 = 8192.
...
bool
SoVulkanRenderBackend::allocateMapDescriptorSet(const VkImageView views[3],
                                                ...
                                                VkDescriptorPool & pool)
{
  ...
  ai.descriptorPool = this->descriptorPool;
  ai.descriptorSetCount = 1;
  ai.pSetLayouts = &this->mapSetLayout;
  if (vkAllocateDescriptorSets(this->device, &ai, &set) != VK_SUCCESS) {
    return false;
  }
  ++this->descriptorSetCount;
  pool = this->descriptorPool;
  ...
  vkUpdateDescriptorSets(this->device, 3, writes, 0, nullptr);
  return true;
}
                                          ...
                                          const uint32_t newSlotsPerFrame)
{
  ...
  // The swap below must still complete (the descriptor sets have to be
  // repointed at the new buffer before the old one is released), so record the
  // failure and return it at the end instead of bailing out here.
  const bool instanceRingOk = this->ensureInstanceModelRingCapacity();
  if (!instanceRingOk) {
...
{
  // The sets are only returned to their pools after the batch ring wraps back
  // around: a pending frame may still reference them, and vkFreeDescriptorSets
  // on an in-use set is a spec violation.  Pools are append-only (never
  // reset), so the pool handles captured here stay valid until shutdown.
  if (entry.mapDescriptorSet != VK_NULL_HANDLE) {
    const VkDescriptorPool pool = entry.mapDescriptorPool;
    const VkDescriptorSet set = entry.mapDescriptorSet;
    this->deferDestroy([this, pool, set]() {
      this->freeDescriptorSet(pool, set,
                              "deferDestroyTextureEntry (map set)");
    });
    if (this->descriptorSetCount > 0) --this->descriptorSetCount;
  ...
  if (entry.descriptorSet != VK_NULL_HANDLE) {
    ...
    this->deferDestroy([this, pool, set]() {
      this->freeDescriptorSet(pool, set,
                              "deferDestroyTextureEntry (texture set)");
    });
    if (this->descriptorSetCount > 0) --this->descriptorSetCount;
  }
  ...
  const auto deferImage = [device, allocator, vmaAllocator, this](
                            const VkImage image, const VmaAllocation allocation,
  ...
  deferImage(entry.image, entry.allocation, entry.view);
  for (VulkanCachedMap & map : entry.maps) {
    deferImage(map.image, map.allocation, map.view);
  }
...
SoVulkanRenderBackend::createPipelineLayout()
{
  ...
  // Set 0 = lighting constant, set 1 = per-draw UBO + base texture,
  // set 2 = optional PBR maps (roughness/normal/emissive).
  ...
  uint32_t setCount = 0;
  if (this->lightingSetLayout != VK_NULL_HANDLE) ++setCount;
  if (this->descriptorSetLayout != VK_NULL_HANDLE) ++setCount;
  if (this->mapSetLayout != VK_NULL_HANDLE) ++setCount;
                                          ...
                                          VkShaderModule & module)
{
  ...
  ci.pCode = code;
  const VkResult createRes =
    vkCreateShaderModule(this->device, &ci, this->allocator, &module);
  if (createRes != VK_SUCCESS) {
    this->emitError(("createShaderModule: vkCreateShaderModule failed: "
...
SoVulkanRenderBackend::createSubPixelCullPipeline()
{
  ...
  const VkResult createRes = vkCreateComputePipelines(
    this->device, this->pipelines.handle(), 1, &cpci, this->allocator,
    &this->subPixelCullPipeline);
  if (createRes != VK_SUCCESS) {
    // Optional acceleration; log (not error) with the reason, and the caller
    // keeps the full-draw path.
    this->subPixelCullPipeline = VK_NULL_HANDLE;
    this->emitLog(("createSubPixelCullPipeline: vkCreateComputePipelines failed: "
                   + SoVulkanShared::vkResultName(createRes)).c_str());
    return false;
  }
