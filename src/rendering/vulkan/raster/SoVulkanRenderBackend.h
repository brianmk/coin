// src/rendering/vulkan/raster/SoVulkanRenderBackend.h
...
#include "rendering/vulkan/raster/SoVulkanRecordContext.h"
...
#include "rendering/vulkan/common/memory/SoVulkanFrameRing.h"
...
#include "rendering/vulkan/common/caches/SoVulkanGeometryCache.h"
...
private:
  ...
  bool allocateTextureDescriptorSet(VkImageView view, VkSampler sampler,
                                    VkDescriptorSet & set);
  // Allocate a set-2 descriptor binding the three optional PBR maps
  // (roughness, normal, emissive) from the shared pool.  `pool` receives the
  // pool the set was allocated from (the active one), so the caller can free
  // it back correctly after the pool has since advanced.
  bool allocateMapDescriptorSet(const VkImageView views[3],
  ...
  // Set-2 layout: the three optional PBR maps (roughness/normal/emissive),
  // bound together on every draw (defaults when absent).
  VkDescriptorSetLayout mapSetLayout = VK_NULL_HANDLE;
  ...
  // Serializes the pipeline store's cold path (find + insert).  The parallel
  // record workers normally hit the per-command warm fast path unlocked, but
  // an un-warmed key (e.g. the batch-fallback path) can reach the cache from
  // several workers at once; concurrent find/insert on the store's map is a
  // data race.  The warm-up in buildWorkItems() is a performance optimization,
  // not a correctness guarantee, so the store is guarded independently.
