// src/rendering/vulkan/common/memory/SoVulkanStagingPool.cpp
...
SoVulkanStagingPool::ensureCapacity(VkDeviceSize required)
{
  ...
  if (!SoVulkanShared::createVmaBuffer(this->allocator_, desc, "staging pool",
                                       newBuffer, newAllocation, &allocationInfo)
         .isOk()) {
    return false;
  }
